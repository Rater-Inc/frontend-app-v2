...
const RatingPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!spaceId) return;
      const spaceData = await api.getSpace(spaceId,spaceAuth.getToken(spaceId));
      if(spaceData){
  ...
  const handleRatingChange = (metricId: number, value: number) => {
    ...
    setRatings(prev => {
      const updatedRatings = prev.filter(r => !(r.metricId === metricId && r.rateeId === currentParticipant.participantId));
  ...
  const handleSubmit = async () => {
    const ratingTobeSubmitted: Rating = {
      raterNickName: "rater",
    ...
    try {
      if(!spaceId) return;
      const response = await submitRatings(ratingTobeSubmitted,spaceAuth.getToken(spaceId));
      if(response){
  ...
  // const hasAllRatings = metrics.every(
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl">
        ...
        <div className="p-6">
          <div className="text-center mb-8">
            <h3 className="text-xl font-semibold text-gray-900">{currentParticipant?.participantName}</h3>
          </div>
          ...
          <div className="space-y-8">
            {metrics.map(metric => (
              <div key={metric.metricId} className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-medium">
                        {ratings.find(e => e.metricId === metric.metricId && e.rateeId === currentParticipant?.participantId)?.score || 0}
                    </span>
                ...
                <RatingSlider
                  max={5}
                  value={ratings.find(e => e.metricId === metric.metricId && e.rateeId === currentParticipant?.participantId)?.score || 0}
                  onChange={(value) => handleRatingChange(metric.metricId, value)}
                />
              </div>
            ))}
          </div>
