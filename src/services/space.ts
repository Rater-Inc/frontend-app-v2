import { SpacexData, Rating , SpaceResultData } from '../types/types';
import { api, submitRatings } from './api';
import { storage } from './storage';

export const space = {
  getSpace: async (spaceId: string, token: string): Promise<SpacexData> => {
    try {
      const spaceInfo = storage.getSpaceInfo();
      const spaceIdInfo = spaceInfo[spaceId]?.spaceData;
      if (spaceIdInfo) {
        return spaceIdInfo as SpacexData;
      }
      ...
      if (response?.status === 200) {
        ...
        return spaceData as SpacexData;
      }
      throw new Error('Invalid space');
    } catch (error) {
      console.error('Error getting space:', error);
      return {} as SpacexData;
    }
  },
